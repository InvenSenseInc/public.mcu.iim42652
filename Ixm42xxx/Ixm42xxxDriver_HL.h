...
/** @defgroup DriverIxm42xxxDriver_HL Ixm42xxx driver high level functions
 *  @brief High-level function to setup an Ixm42xxx device
 *  @ingroup  DriverIxm42xxx
 *  @{
 */

/** @file Ixm42xxxDriver_HL.h
 * High-level function to setup an Ixm42xxx device
 */

#ifndef _INV_IXM42XXX_DRIVER_HL_H_
#define _INV_IXM42XXX_DRIVER_HL_H_
 ...
 */
#ifndef INV_IXM42XXX_LIGHTWEIGHT_DRIVER
	#define INV_IXM42XXX_LIGHTWEIGHT_DRIVER 0
#endif

/** @brief Scale factor and max ODR 
 *  Dependant of chip
 */
#if (defined(ICM42600) || defined(ICM42622) || defined(ICM42686) || defined(ICM42688))
	/* 
	...
	#define ICM_PART_DEFAULT_OIS_MODE IXM42XXX_SENSOR_CONFIG2_OIS_MODE_32KHZ
#else
	#define PLL_SCALE_FACTOR_Q24 (1UL<<24)
	#define ICM_PART_DEFAULT_OIS_MODE IXM42XXX_SENSOR_CONFIG2_OIS_MODE_8KHZ
#endif
 ...
 */
#if defined(ICM42686)
	#define ACCEL_CONFIG0_FS_SEL_MAX IXM42XXX_ACCEL_CONFIG0_FS_SEL_32g
	#define GYRO_CONFIG0_FS_SEL_MAX  IXM42XXX_GYRO_CONFIG0_FS_SEL_4000dps

	#define ACCEL_OFFUSER_MAX_MG 2000 
	#define GYRO_OFFUSER_MAX_DPS 128 
#else
	#define ACCEL_CONFIG0_FS_SEL_MAX IXM42XXX_ACCEL_CONFIG0_FS_SEL_16g
	#define GYRO_CONFIG0_FS_SEL_MAX  IXM42XXX_GYRO_CONFIG0_FS_SEL_2000dps
...
/** @brief Ixm42xxx maximum buffer size mirrored from FIFO at polling time
 *  @warning fifo_idx type variable must be large enough to parse the FIFO_MIRRORING_SIZE
 */
#define IXM42XXX_FIFO_MIRRORING_SIZE 16 * 129 // packet size * max_count = 2064
...
/** @brief Ixm42xxx Accelerometer start-up time before having correct data
 */
#define IXM42XXX_ACC_STARTUP_TIME_US 20000U

/** @brief Ixm42xxx Gyroscope start-up time before having correct data
 */
#define IXM42XXX_GYR_STARTUP_TIME_US 60000U

/** @brief Sensor identifier for UI control and OIS function
 */
enum inv_ixm42xxx_sensor {
	INV_IXM42XXX_SENSOR_ACCEL,               /**< Accelerometer (UI control path) */
	INV_IXM42XXX_SENSOR_GYRO,                /**< Gyroscope (UI control path) */
	INV_IXM42XXX_SENSOR_FSYNC_EVENT,         /**< Used by OIS and UI control layers */
	INV_IXM42XXX_SENSOR_OIS,                 /**< Only used by OIS layer */
	INV_IXM42XXX_SENSOR_TEMPERATURE,         /**< Chip temperature, enabled by default. However, it will be reported only if Accel and/or Gyro are also enabled. 
	                                              The Temperature's ODR (Output Data Rate) will match the ODR of Accel or Gyro, or the fastest if both are enabled*/
	INV_IXM42XXX_SENSOR_TAP,                 /**< Tap and Double tap */
	INV_IXM42XXX_SENSOR_DMP_PEDOMETER_EVENT, /**< Pedometer: step is detected */
	INV_IXM42XXX_SENSOR_DMP_PEDOMETER_COUNT, /**< Pedometer: step counter */
	INV_IXM42XXX_SENSOR_DMP_TILT,            /**< Tilt */
	INV_IXM42XXX_SENSOR_DMP_FF,              /**< Free Fall */
	INV_IXM42XXX_SENSOR_DMP_LOWG,            /**< Low G */
	INV_IXM42XXX_SENSOR_MAX
};

/** @brief Configure Fifo usage
 */
typedef enum {
	INV_IXM42XXX_FIFO_DISABLED = 0,              /**< Fifo is disabled and data source is sensors registers */
	INV_IXM42XXX_FIFO_ENABLED  = 1,              /**< Fifo is used as data source */
}INV_IXM42XXX_FIFO_CONFIG_t;
...
} inv_ixm42xxx_sensor_event_t;

/** @brief Ixm42xxx driver states definition
 */
struct inv_ixm42xxx {
	struct inv_ixm42xxx_transport transport;                              /**< Warning : this field MUST be the first one of 
	                                                                   struct ixm42xxx */

	void (*sensor_event_cb)(inv_ixm42xxx_sensor_event_t * event); /**< callback executed by inv_ixm42xxx_get_data_from_fifo function
	                                                                   for each data packet extracted from fifo or inv_ixm42xxx_get_data_from_registers read data from register
	                                                                   This field may be NULL if inv_ixm42xxx_get_data_from_fifo/inv_ixm42xxx_get_data_from_registers
	...
	uint8_t fifo_data[IXM42XXX_FIFO_MIRRORING_SIZE];              /**<  FIFO mirroring memory area */
	...
	INV_IXM42XXX_FIFO_CONFIG_t fifo_is_used;                      /**< Data are get from FIFO or from sensor registers. By default Fifo is used*/
...
/** @brief Ixm42xxx set of interrupt enable flag
 */
typedef struct {
 ...
 *  inv_ixm42xxx_serif.
 *
 *  @param[in] serif pointer on serial interface structure to be used to access ixm42xxx.
 *
 *  @param[in] sensor_event_cb callback executed by inv_ixm42xxx_get_data_from_fifo function
 *  each time it extracts some valid data from fifo. Or inv_ixm42xxx_get_data_from_registers read data
 *  from register. Thus this parameter is optional as long
 *  as inv_ixm42xxx_get_data_from_fifo/inv_ixm42xxx_get_data_from_registers function is not used.
 *
 ...
 *  @warning inv_ixm42xxx::register_cache::pwr_mngt_0_reg is modified by this function
 */
 ...
 *  @param[in] resol The expected resolution of the timestamp. See enum IXM42XXX_TMST_CONFIG_RESOL_t.
 *  @return 0 on success, negative value on error.
 ...
 *  sensor_event_cb funtion passed in parameter of inv_ixm42xxx_init function for each packet 
 *  @return 0 on success, negative value on error.
 */
 ...
 *  sensor_event_cb funtion passed in parameter of inv_ixm42xxx_init function for each 
 *  packet.
 *  @return 0 on success, negative value on error.
 */
int inv_ixm42xxx_get_data_from_fifo(struct inv_ixm42xxx * s);

/** @brief Converts IXM42XXX_ACCEL_CONFIG0_ODR_t or IXM42XXX_GYRO_CONFIG0_ODR_t enums to period expressed in us
 *  @param[in] odr_bitfield An IXM42XXX_ACCEL_CONFIG0_ODR_t or IXM42XXX_GYRO_CONFIG0_ODR_t enum
 ...
 *  @sa IXM42XXX_ACCEL_CONFIG0_ODR_t
 *  @return 0 on success, negative value on error.
 *  @warning inv_ixm42xxx::register_cache::accel_cfg_0_reg is modified by this function
 */
 ...
 *  @param[in] frequency The requested frequency.
 *  @sa IXM42XXX_GYRO_CONFIG0_ODR_t
 *  @return 0 on success, negative value on error.
 *  @warning inv_ixm42xxx::register_cache::gyro_cfg_0_reg is modified by this function
 */
 ...
 *  @sa IXM42XXX_ACCEL_CONFIG0_FS_SEL_t.
 *  @return 0 on success, negative value on error.
 *  @warning inv_ixm42xxx::register_cache::accel_cfg_0_reg is modified by this function
 */
 ...
 *  @sa IXM42XXX_ACCEL_CONFIG0_FS_SEL_t.
 *  @return 0 on success, negative value on error.
 *  @warning inv_ixm42xxx::register_cache::accel_cfg_0_reg is relied upon by this function
 */
 ...
 *  @sa IXM42XXX_GYRO_CONFIG0_FS_SEL_t.
 *  @return 0 on success, negative value on error.
 *  @warning inv_ixm42xxx::register_cache::gyro_cfg_0_reg is modified by this function
 */
 ...
 *  @sa IXM42XXX_GYRO_CONFIG0_FS_SEL_t.
 *  @return 0 on success, negative value on error.
 *  @warning inv_ixm42xxx::register_cache::gyro_cfg_0_reg is relied upon by this function
 */
 ...
 *  @sa IXM42XXX_GYRO_ACCEL_CONFIG0_ACCEL_FILT_AVG_t
 *  @return 0 on success, negative value on error.
 */
 ...
 *  @sa IXM42XXX_GYRO_ACCEL_CONFIG0_ACCEL_FILT_BW_t
 *  @return 0 on success, negative value on error.
 */
...
/** @brief Get the timestamp value of ixm42xxx from register
 *  @param[in] icm_time timestamp read from register
 *  @return 0 on success, negative value on error.
 *  @warning Prior to call this API, the read access to timestamp register must be enabled (see @ref inv_ixm42xxx_enable_timestamp_to_register() function)
 */
 ...
 *  @return 0 on success, negative value on error.
 *  @warning In case CLKIN is disabled, it is recommended to call inv_ixm42xxx_configure_timestamp_resolution()
 *  just afterwards so that timestamp resolution is in line with system request
 *  @warning inv_ixm42xxx::register_cache::intf_cfg_1_reg is modified by this function
 */
 ...
 *  @warning In case CLKIN is disabled, it is recommended to call inv_ixm42xxx_configure_timestamp_resolution()
 *  just afterwards so that timestamp resolution is in line with system request
 *  @warning inv_ixm42xxx::register_cache::intf_cfg_1_reg is relied upon by this function
 */
 ...
 *  @param[in] dec_rate decimation value from 0 to 127
...
#endif /* _INV_IXM42XXX_DRIVER_HL_H_ */
